...
不需要任何金融知识。Windows 上**双击项目里的 `start.cmd`**：第一次会自动安装依赖，然后打开两个小窗口（**引擎**和**应用**）并在浏览器打开 `http://127.0.0.1:5173`。

在主界面点那颗会发光的**绿色 ▶ 开始键**：AI 就会自动找策略、用 20 年历史回测、把胜出的策略模拟投资——同时让约 10 条策略同场竞速，并自动淘汰落后的。打开「策略赛马」标签即可观看。你可以随时关掉浏览器，比赛会在引擎窗口里继续；关掉那两个窗口即可停止。

想手动运行：

```
npm install
npm run dialogue-bridge      # 引擎（保持开着）。要做模拟交易：先把 QRL_ALPACA_KEY_FILE 指向你的 Alpaca paper 密钥文件
npm run dev                  # 应用 → 打开 http://127.0.0.1:5173
```

全程都是**模拟盘 / 虚拟资金**，任何地方都没有真金白银的通路。免费的 Alpaca paper 账户（[alpaca.markets](https://alpaca.markets)）是可选的，只有在你想让它真的下模拟单时才需要；没有密钥它照样研究、回测、赛马。
...
不需要金融知识。启动引擎+应用后，在主界面按那颗会发光的**绿色 ▶**。这一个按钮把整件事作为**一个进程**跑起来：

1. **并行研究**策略（Claude Code，Opus→Sonnet 自动回退，触发限流就睡到额度恢复再继续）；
2. 用 20 年历史**回测**每一条（无未来数据、前进检验、deflated Sharpe、样本外 IC），只留下胜出的；
3. 把它们作为一场**策略赛马**模拟投资：约 10 条策略各占账户一格，用实时价格盯市；每隔几小时淘汰最差的一匹、换上新近通过验证的挑战者；领跑策略的持仓镜像到你的 Alpaca 模拟账户。

你看到的办公室研究员，是这个**唯一进程的实时镜像**——应用不会为同一件事再调用一次大模型。

主界面的标签页：

- **🏇 策略赛马** —— 实时排行榜（每条策略的血统、净值、持仓、淘汰记录）。
- **📈 模拟交易** —— 先用历史回测验证策略，通过才下单，并查看它最近 **1 / 5 / 10 天**的模拟盈亏。
- **🔬 策略库** —— 点开任意策略，看它的信号怎么推导（现象 → 信号 → 中性化 → 排序 → 持有），还能用一句「老板指令」改造它。

## 实测 vs 示意

一轮资深量化评审（3 轮，**4.5 → 8.4 → 9.0 / 10**）加一次自查，把计算层逼到对自己的边界诚实（完整记录见 [docs/REVIEW.md](docs/REVIEW.md)）：

- **从数据实测**——无未来数据的横截面回测（bar `t` 信号赚 `t+1`）、信号先去极值+行业/beta 中性化再排序、按换手计成本、Sharpe/Sortino/Calmar/PSR、Alphalens IC（准入门槛用更严格的**样本外 OOS IC**，不回退到全样本）、deflated Sharpe、净化+隔离的前进检验、按**基准**而非策略自身盈亏划分市场状态、以及一个**实测的随机基线**。每个底层公式都对照 Python 的 `empyrical`/`scipy`/`statsmodels` 校验过。
- **容量已实测**——内置数据带 **OHLCV（成交量 + 复权高低价）**，最大可部署资金由真实**中位 ADV × 5% 参与率 ÷ 换手**算出；并提供成交量因子（Amihud 非流动性、低换手、Parkinson 区间波动）。约 **513 只** S&P 500 + 纳指股票池。
- **仍是示意**——执行 / 延迟 / 部分成交等数字（没有微观结构数据）；只有收盘价的上传数据会退回示意脚手架。
- **不可晋升**——当前数据无法真正回测的家族（如纯价格数据上的新闻 / 财报因子）只用模拟器出示意曲线，永不计入打分、池、净值或晋升。
...
- [x] **一键自动驾驶**：一颗绿色 ▶ 把「研究 → 回测 → 模拟投资」作为**一个进程**跑起来；办公室动画是它的实时镜像（不会重复调用大模型）。`start.cmd` 一键拉起引擎+应用。
- [x] **策略赛马**：约 10 条策略并行研究、验证，作为虚拟分仓用实时价格同场竞速；淘汰最差、换上通过验证的挑战者；领跑策略镜像到你的 Alpaca 模拟账户。
- [x] **应用内模拟交易**（Alpaca，仅 paper 端点）：历史验证→通过才下单，带 1 / 5 / 10 天盈亏；外加可查看并改造每条策略逻辑的「策略库」。
- [x] **经校验的计算内核**：无未来数据回测（去极值 + 行业/beta 中性化信号）、净化+隔离前进检验、deflated Sharpe、**样本外 IC 准入门槛**、实测随机基线——全部对照 Python `empyrical`/`scipy`/`statsmodels` 校验。评审 **9/10**（[docs/REVIEW.md](docs/REVIEW.md)）。
- [x] **OHLCV 数据**：成交量 + 复权高低价 → Amihud 非流动性、低换手、区间波动因子，以及一个**实测**容量模型（中位 ADV）；约 513 只 S&P 500 + 纳指股票池。
- [x] Research Workflow 2.0：发现卡片、编译信号、来源可信度、novelty、point-in-time 合同、验证、基线和研究 feed。
- [x] 通过本地桥接器使用 Claude Code / Codex 作为研究大脑（Opus→Sonnet 回退 + 限流恢复处理）。
- [x] 自带数据：上传、远程 URL、大型本地文件和数据库；tick→月频率感知指标；大型数据源缓存内核。
- [x] Thompson bandit、池级 delta-Sharpe 奖励、MAP-Elites 生态位、CSCV PBO。
- [x] 游戏层：经验值、头衔、成就、基金净值、办公室事件、彩带、EN / 中文。

下一步想法：打包成单文件桌面应用（引擎+界面合到一个窗口）、质量家族接入基本面数据、中小盘股票池去存活偏差。
